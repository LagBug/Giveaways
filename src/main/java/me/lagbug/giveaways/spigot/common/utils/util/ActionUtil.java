package me.lagbug.giveaways.spigot.common.utils.util;
...
public class ActionUtil {
	...
	public static boolean execute(OfflinePlayer player, List<String> actions) {
		...
		List<String> actionsCopy = new ArrayList<String>(Arrays.asList(actions.toArray(new String[0]).clone()));
		...
		for (int i = 0; i < actions.size(); i++) {
			...
			/*
			 * if (CommonUtils.isPluginEnabled("PlaceholderAPI")) { action =
			...
			switch (type) {
			case "DELAY":
				Bukkit.getScheduler().runTaskLater(CommonUtils.getPlugin(), new Runnable() {
					@Override
					public void run() {
						execute(player, actionsCopy);
					}
			...
			case "TEMPBAN":		
			    ...
			    date.add(Calendar.SECOND, Integer.valueOf(actionSplited[2]));
