package me.lagbug.giveaways.spigot.common.builders;
	...
	public void update() {
		...
		for (String key : config.getConfigurationSection("contents").getKeys(false)) {
			...
			for (String subKey : toReplace.keySet()) {
				...
				/*
				 * if (CommonUtils.isPluginEnabled("PlaceholderAPI")) { name =
				 * PlaceholderAPI.setPlaceholders(getPlayer(), name); }
				 */
			...
			/*
			 * if (CommonUtils.isPluginEnabled("PlaceholderAPI")) { for (int i = 0; i <
			 * lore.size(); i++) { lore.set(i, PlaceholderAPI.setPlaceholders(getPlayer(),
			 * lore.get(i))); } }
			 */
